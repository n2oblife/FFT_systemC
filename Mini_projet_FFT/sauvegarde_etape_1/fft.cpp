...
void FFT::COMPORTEMENT()
{
    ...
    { 
              ...
              // TEST BCA A FAIRE
	    {
            ...
            // FIFO :
            cout<< "Lecture des fichiers ! \n" << endl;
            for (i = 0; i < 8; i++) // 8 couples de données pour commencer FFT
            { 
                cx_in[i].real = in.read();
                //wait();
                cx_in[i].imag = in.read();
                //wait();
            ...
            // ENLEVER EMISSION DES OUT DU IF ?
            // TEST BCA A FAIRE
            for (i = 0; i < 8; i++) // 8 couples de données vers SINK
            { 
                out.write(cx_out[i].real);
                out.write(cx_out[i].imag);
